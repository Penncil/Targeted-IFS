...
result1<-readRDS(file="adap_n300p500_Ah10_heter.rds")
...
index<-which(res$Ah>5 | res$method=="Trans")
...
plot <- plot + 
  ...
  facet_manual(vars(type),design =matrix(c(1:3),1,3),
               strip.position = "left",
               scales = "free_y")+
  labs(x = "Number of transferable source", y="")+
...
result1<-readRDS(file="adap_n300p500_Ah20_heter.rds")
